          ...
          <div className="mb-6 p-4 bg-slate-700/50 rounded-lg">
            ...
            <div className="flex items-center gap-1 mb-3">
              {[1, 2, 3, 4, 5].map((star) => {
                ...
                const isFilled = submission.grade! >= starValue;
                return (
                  <svg
                    key={star}
                    className={`w-6 h-6 ${isFilled ? 'text-yellow-400' : 'text-slate-600'}`}
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
              ...
              <span className="ml-2 text-sm text-slate-400">
                ({Math.round(submission.grade! / 20)} de 5 estrellas)
              </span>
            </div>

            {/* Información del tutor que calificó */}
