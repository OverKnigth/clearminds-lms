    ...
    <div className="min-h-screen bg-slate-900 pt-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        ...
        <div className="mb-6 sm:mb-8">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3 sm:gap-4">
              <div className="w-12 h-12 sm:w-14 sm:h-14 bg-red-500/20 rounded-2xl flex items-center justify-center flex-shrink-0">
              ...
              <div>
                <h1 className="text-2xl sm:text-3xl font-bold text-white">Mis Tutorías</h1>
                <p className="text-sm sm:text-base text-slate-400">Gestiona tus sesiones de validación con tutores</p>
              </div>
            </div>
            <button
              onClick={() => setShowRequestModal(true)}
              className="w-full sm:w-auto px-4 sm:px-6 py-2.5 sm:py-3 bg-gradient-to-r from-red-600 to-red-700 hover:from-red-500 hover:to-red-600 text-white text-sm sm:text-base font-semibold rounded-xl transition-all flex items-center justify-center gap-2"
            >
              <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {pendingTutorings.length > 0 && (
          <div className="mb-8 sm:mb-12">
            <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
              <h2 className="text-xl sm:text-2xl font-bold text-white">Pendientes de Confirmar</h2>
              <span className="px-2 sm:px-3 py-1 bg-yellow-500/20 text-yellow-400 text-xs sm:text-sm font-semibold rounded-full">
        ...
        {scheduledTutorings.length > 0 && (
          <div className="mb-8 sm:mb-12">
            <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
              <h2 className="text-xl sm:text-2xl font-bold text-white">Próximas Tutorías</h2>
              <span className="px-2 sm:px-3 py-1 bg-red-500/20 text-red-400 text-xs sm:text-sm font-semibold rounded-full">
        ...
        {completedTutorings.length > 0 && (
          <div className="mb-8 sm:mb-12">
            <div className="flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
              <h2 className="text-xl sm:text-2xl font-bold text-white">Tutorías Completadas</h2>
    ...
    <div className="bg-slate-800 rounded-xl p-4 sm:p-6 border border-slate-700 hover:border-slate-600 transition-all">
      <div className="flex flex-col sm:flex-row items-start justify-between mb-4 gap-3">
        <div className="flex-1 w-full min-w-0">
          <h3 className="font-semibold text-white mb-1 truncate">{tutoring.blockName}</h3>
          <p className="text-xs sm:text-sm text-slate-400 mb-3 truncate">{tutoring.courseName}</p>
      ...
      <div className="space-y-2 mb-4">
        {tutoring.tutorName && (
          <div className="flex items-center gap-2 text-xs sm:text-sm text-slate-300">
            ...
            <span className="truncate">{tutoring.tutorName}</span>
          </div>
        )}
        {tutoring.scheduledDate && (
          <div className="flex items-center gap-2 text-xs sm:text-sm text-slate-300">
            ...
            <span className="truncate">{new Date(tutoring.scheduledDate).toLocaleDateString()} - {tutoring.scheduledTime}</span>
          </div>
        )}
        <div className="flex items-center gap-2 text-xs sm:text-sm text-slate-300">
          <svg className="w-3 h-3 sm:w-4 sm:h-4 text-slate-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <span className="truncate">Solicitada: {new Date(tutoring.requestDate).toLocaleDateString()}</span>
        </div>
      </div>

      {showResults && tutoring.grade !== undefined && (
        <div className="mb-4 p-3 sm:p-4 bg-slate-700/50 rounded-lg">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs sm:text-sm text-slate-300">Calificación:</span>
            <span className={`text-xl sm:text-2xl font-bold ${tutoring.approved ? 'text-green-400' : 'text-red-400'}`}>
              {tutoring.grade}/10
            </span>
          </div>
          {tutoring.feedback && (
            <div className="mt-3 pt-3 border-t border-slate-600">
              ...
              <p className="text-xs sm:text-sm text-slate-300">{tutoring.feedback}</p>
            </div>
          )}
          {tutoring.recordingLink && (
            <a
              ...
              className="mt-3 flex items-center gap-2 text-red-400 hover:text-red-300 text-xs sm:text-sm"
            >
      ...
      {showJoinButton && tutoring.meetingLink && (
        <div className="flex gap-3 pt-4 border-t border-slate-700">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="flex-1 py-2 sm:py-2.5 bg-gradient-to-r from-red-600 to-red-700 hover:from-red-500 hover:to-red-600 text-white text-sm font-semibold rounded-lg transition-all text-center"
          >
            Unirse a la Reunión
          </a>
        </div>
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-slate-800 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-slate-700">
        <div className="p-4 sm:p-6 border-b border-slate-700 flex items-center justify-between sticky top-0 bg-slate-800 z-10">
          <h2 className="text-lg sm:text-xl font-bold text-white">Solicitar Tutoría</h2>
