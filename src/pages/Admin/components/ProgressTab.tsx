...
export function ProgressTab() {
  ...
  const exportGroupToExcel = () => {
    ...
    const excelData = groupData.map(row => ({
      'Estudiante': row.studentName,
      'Curso': row.courseName || '-',
      'Progreso (%)': row.progress,
      'Bloques': row.blocks && row.blocks.length > 0 ? row.blocks.map((b: any) => `${b.name} (${b.approved ? 'Aprobado' : 'Pendiente'})`).join(', ') : '-',
      'Sesiones de Tutoría': row.tutorings ? row.tutorings.length : 0,
      'Insignias': row.badges && row.badges.length > 0 ? row.badges.join(', ') : '-',
      'Nota': row.grade ? Number(row.grade).toFixed(1) : '-'
    }));

    const worksheet = XLSX.utils.json_to_sheet(excelData);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Reporte Grupo');
    XLSX.writeFile(workbook, `Reporte_Grupo_${selectedGroup}.xlsx`);
  };
                ...
                <div className="flex items-center gap-4 bg-slate-800/50 p-4 rounded-xl border border-slate-700/50">
                  ...
                  {groupData.length > 0 && (
                    <button 
                      ...
                      className="ml-auto px-6 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg font-black text-xs uppercase tracking-widest transition-all flex items-center gap-2 shadow-lg shadow-emerald-900/20"
                    >
