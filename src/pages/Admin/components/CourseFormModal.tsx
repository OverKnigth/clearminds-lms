...
export function CourseFormModal({ isOpen, onClose, onSubmit, course, tutors }: CourseFormModalProps) {
  ...
  useEffect(() => {
    if (course) {
      ...
      // Supabase devuelve URLs públicas completas
  ...
  const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Validar que haya un nombre de curso
    if (!formData.name || formData.name.trim() === '') {
      alert('Por favor ingresa el nombre del curso antes de subir la imagen');
      return;
    }
    ...
    try {
      ...
      // Subir imagen al servidor con el nombre del curso (que la subirá a Supabase Storage)
      const response = await api.uploadCourseImage(file, formData.name);
      if (response.success) {
        const publicUrl = response.data.imageUrl;
        setFormData(prev => ({ ...prev, imageUrl: publicUrl }));
        setImagePreview(publicUrl); // URL pública completa de Supabase
      }
    } catch (error: any) {
