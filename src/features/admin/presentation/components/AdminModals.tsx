...
import type { Student, CourseData, FormData, ContentFormData } from '../../domain/entities';
import type { Group, GroupDetail } from '@/shared/types/group';
        ...
        {modalType === 'assignCourse' && selectedStudent && (
          <form onSubmit={handleAssignCourses} className="space-y-4">
            ...
            <div className="relative z-50">
              <label htmlFor="groupFilter" className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 cursor-pointer">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  // Use functional update to ensure we don't use stale formData
                  setFormData({ ...formDataRef.current, groupId: val, selectedCourses: [] });
                  ...
                  api.getGroupDetail(val)
                    .then((res) => {
                      ...
                      if (groupCourseIds.length > 0) {
                        setFormData({ ...formDataRef.current, groupId: val, selectedCourses: groupCourseIds });
                      }
                    })
                    .catch(() => {
                      // silently fail
                ...
                className="w-full px-4 py-2.5 bg-slate-700 border border-slate-600 rounded-lg text-white text-xs font-bold focus:outline-none focus:ring-2 focus:ring-red-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all cursor-pointer relative z-50"
                disabled={loadingGroups}
              >
            ...
            <div className="flex gap-3 pt-2">
              <button type="submit"
                className="flex-1 py-2.5 bg-gradient-to-r from-red-600 to-red-700 hover:from-red-500 hover:to-red-600 text-white text-xs font-black uppercase tracking-widest rounded-lg transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                disabled={formData.selectedCourses.length === 0 || submitting}>
                ...
                {submitting ? 'Asignando...' : `Asignar (${formData.selectedCourses.length})`}
              </button>
              <button type="button" onClick={() => setIsModalOpen(false)} disabled={submitting}
                className="px-5 py-2.5 bg-slate-700 hover:bg-slate-600 text-white text-xs font-black uppercase tracking-widest rounded-lg transition-colors disabled:opacity-50">
                Cancelar
              </button>
            </div>
          </form>
...
function EditStudentForm({
  ...
  const [selectedGroupId, setSelectedGroupId] = useState(formData.groupId || '');
  const [loadingGroups, setLoadingGroups] = useState(false);
  const [loadingDetail, setLoadingDetail] = useState(false);
  const formDataRef = React.useRef(formData);
  formDataRef.current = formData;
  ...
  // Load group detail when user selects a group or on mount
  useEffect(() => {
    if (!selectedGroupId) {
      setGroupDetail(null);
      setFormData({ ...formDataRef.current, groupId: '' });
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
          ...
          <select
            ...
            onChange={(e) => {
              ...
              setFormData({ 
                ...formData, 
                groupId: val, 
                courseId: val ? formData.courseId : '',
                selectedCourses: val ? formData.selectedCourses : [],
                courseParallelMap: val ? formData.courseParallelMap : {}
              });
            }}
            disabled={loadingGroups}
          >
            {loadingGroups && <option value="">Cargando...</option>}
            {!loadingGroups && !formData.groupId && <option value="">-- Sin Grupo --</option>}
            
            {/* Si el ID de grupo asignado no está en la lista (es una Cohorte o Grupo inactivo), lo forzamos a aparecer */}
            {!loadingGroups && formData.groupId && !groupsList.some(g => g.id === formData.groupId) && (
              <option value={formData.groupId}>{(formData as any).groupName || 'Grupo asignado'} (Actual)</option>
            )}

            {!loadingGroups && groupsList.map((g) => (
              <option key={g.id} value={g.id}>{g.name}</option>
            ))}
          </select>
        </div>
      ...
      {/* Course selector — Checkbox list */}
      {userRole?.toLowerCase() === 'student' && selectedGroupId && (
        <div className="space-y-3">
          <label className={LABEL}>Cursos Asignados</label>
          <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-4 max-h-60 overflow-y-auto custom-scrollbar">
            {loadingDetail ? (
              <div className="flex items-center justify-center py-4 text-slate-400 gap-2">
                <svg className="w-4 h-4 animate-spin text-red-500" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z" /></svg>
                <span className="text-[10px] font-bold uppercase tracking-widest">Cargando cursos...</span>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {groupDetail?.courses.map((c: any) => {
                  const isSelected = formData.selectedCourses.includes(c.course.id);
                  return (
                    <div 
                      key={c.course.id}
                      onClick={() => toggleCourseSelection(c.course.id)}
                      className={`
                    ...
                    >
                      ...
                      <span className={`text-[11px] font-bold uppercase tracking-tight transition-colors ${isSelected ? 'text-white' : 'text-slate-300'}`}>
                        {c.course.name}
                      </span>
                    </div>
                  );
                })}
                {(!groupDetail?.courses || groupDetail.courses.length === 0) && (
                  <div className="col-span-full py-4 text-center text-slate-500 text-[10px] uppercase font-bold tracking-widest italic">
                    Este grupo no tiene cursos habilitados
                  </div>
                )}
              </div>
            )}
