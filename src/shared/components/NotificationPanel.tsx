  ...
  const dropdown = isOpen ? (
    ...
    >
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-800/80">
        ...
        {unreadCount > 0 && (
          <button onClick={onMarkAllRead} className="text-[9px] font-black text-slate-600 hover:text-slate-300 uppercase tracking-widest transition-colors">
            Leer todas
          </button>
        )}
      </div>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-baseline justify-between gap-2 mb-0.5">
                  ...
                  <span className="text-[9px] text-slate-700 flex-shrink-0 font-medium">{timeAgo(n.created_at)}</span>
                </div>
                <p className="text-[10px] text-slate-500 leading-relaxed line-clamp-2">{n.body}</p>
              </div>
              {!n.read && <div className="w-1.5 h-1.5 rounded-full bg-red-500 flex-shrink-0 mt-1" />}
            </button>
          ))
        )}
      </div>
